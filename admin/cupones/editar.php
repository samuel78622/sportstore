...
$cupon = obtenerCupon($id);

if (!$cupon) {
    header("Location: index.php?error=cupon_no_encontrado");
    exit();
}
...
<head>
    ...
    <title>Editar Cupón — SportStore Admin</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
    <link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css" rel="stylesheet">
     <link rel="stylesheet" href="../assets/css/cupones/editar.css">
</head>
<body>
...
<div class="main-content">
    ...
    <div class="page-header">
        <div>
            <h1><i class="fas fa-pen me-2"></i>Editar Cupón</h1>
            <small class="text-muted">
                Código: <strong><?= htmlspecialchars($cupon['codigo']) ?></strong>
            </small>
        </div>
        ...
        <div class="row g-4">
            ...
            <div class="col-md-8">
                ...
                <div class="card-section">
                    ...
                    <?php $tipo_actual = $_POST['tipo'] ?? $cupon['tipo']; ?>
                    <div class="tipo-options">
                        <label class="tipo-option <?= $tipo_actual === 'porcentaje' ? 'selected' : '' ?>">
                            <input type="radio" name="tipo" value="porcentaje"
                                <?= $tipo_actual === 'porcentaje' ? 'checked' : '' ?>>
                            <div class="tipo-icon">%</div>
                            <div class="tipo-label">Porcentaje</div>
                            <div class="tipo-desc">Descuento en % sobre el total</div>
                        </label>
                        <label class="tipo-option <?= $tipo_actual === 'monto_fijo' ? 'selected' : '' ?>">
                            <input type="radio" name="tipo" value="monto_fijo"
                                <?= $tipo_actual === 'monto_fijo' ? 'checked' : '' ?>>
                            <div class="tipo-icon">$</div>
                ...
                <div class="card-section">
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            ...
                            <input
                                ...
                                id="inputCodigo"
                                class="form-control"
                                value="<?= htmlspecialchars($_POST['codigo'] ?? $cupon['codigo']) ?>"
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="input-group">
                                <span class="input-group-text" id="simbolo-descuento">
                                ...
                                <input
                                    ...
                                    id="inputDescuento"
                                    class="form-control"
                                    value="<?= htmlspecialchars($_POST['descuento'] ?? $cupon['descuento']) ?>"
                                    min="0"
                                    step="0.01"
                        ...
                        <div class="col-md-6">
                            ...
                            <input
                                ...
                                name="usos_maximos"
                                class="form-control"
                                value="<?= htmlspecialchars($_POST['usos_maximos'] ?? $cupon['usos_maximos']) ?>"
                                min="<?= $cupon['usos_actuales'] ?>"
                                required
                            >
                            <small class="text-muted">
                                Mínimo <?= $cupon['usos_actuales'] ?> (usos ya realizados)
                            </small>
                        </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <input
                                ...
                                id="inputFecha"
                                class="form-control"
                                value="<?= htmlspecialchars($_POST['fecha_vencimiento'] ?? $cupon['fecha_vencimiento'] ?? '') ?>"
                                oninput="actualizarPreview()"
                            >
                        </div>
                    </div>
                    ...
                    <div class="usos-info">
                        <p>Usos actuales: <strong><?= $cupon['usos_actuales'] ?> / <?= $cupon['usos_maximos'] ?></strong></p>
                        <div class="usos-barra">
                            <div class="usos-fill"
                                 style="width:<?= $cupon['usos_maximos'] > 0 ? ($cupon['usos_actuales'] / $cupon['usos_maximos']) * 100 : 0 ?>%">
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="col-md-4">
                <div class="card-section">
                    ...
                    <div class="cupon-preview">
                        ...
                        <div class="preview-codigo" id="prev-codigo">
                            <?= htmlspecialchars($cupon['codigo']) ?>
                        </div>
                        <div class="preview-descuento" id="prev-descuento">
                            <?= $cupon['tipo'] === 'porcentaje'
                                ? $cupon['descuento'] . '%'
                                : '$' . number_format($cupon['descuento'], 0) ?>
                        </div>
                        <div class="preview-tipo" id="prev-tipo">
                            <?= $cupon['tipo'] === 'porcentaje'
                                ? 'de descuento en tu compra'
                                : 'de descuento directo' ?>
                        </div>
                        <div class="preview-vence" id="prev-vence">
                            <?= $cupon['fecha_vencimiento']
                                ? 'Válido hasta: ' . date('d/m/Y', strtotime($cupon['fecha_vencimiento']))
...
<script>
    ...
    function actualizarPreview() {
        ...
        document.getElementById('prev-vence').textContent =
            fecha ? 'Válido hasta: ' + new Date(fecha + 'T00:00:00').toLocaleDateString('es-CO')
                  : 'Sin fecha de vencimiento';
    }
</script>
</body>
